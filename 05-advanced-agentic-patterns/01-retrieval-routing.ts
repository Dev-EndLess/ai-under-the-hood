...
import { z } from "zod";
import { documentationDB, catalogDB } from "./utils/mock-db-data";
    ...
    tools: {
      searchDocumentation: tool({
        ...
        execute: async ({ parameterQuery }) => {
          console.log(
            `🛠️  [Router] -> Scelta: DOCUMENTAZIONE | Filtro: "${parameterQuery}"`,
          );

          const results = documentationDB.filter(
            (doc) =>
      ...
      searchCatalog: tool({
        ...
        execute: async ({ parameterQuery }) => {
          console.log(
            `💰 [Router] -> Scelta: CATALOGO | Filtro: "${parameterQuery}"`,
          );

          const results = catalogDB.filter(
            (item) =>
